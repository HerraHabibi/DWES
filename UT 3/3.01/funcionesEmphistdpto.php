<?php
  ...
  function visualizarTrabajadores($codDpto) {
    $sql = "SELECT DISTINCT emple_dpto.dni, emple.nombre
              FROM emple_dpto
              INNER JOIN emple ON emple_dpto.dni = emple.dni
              WHERE emple_dpto.cod_dpto = '$codDpto'
              AND emple_dpto.fecha_fin IS NOT NULL";
    $resultado = operarBd($sql);

    if (empty($resultado))
      echo 'Aún no han habido trabajadores que hayan dejado este departamento';
